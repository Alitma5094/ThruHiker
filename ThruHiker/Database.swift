...
class Database: ObservableObject {
    ...
    func loadSavedLocations(inputLocations: [TrailLocationSave]) -> [TrailLocation] {
        var items = [TrailLocation]()
        
        do {
            for site in inputLocations {
                let table = Table(site.unwrappedTypeString.capitalized + "s")
                let query = table.filter(Expression<String>("OBJECTID") == site.unwrappedObjectid)
                ...
                for campsite in try connection.prepare(query) {
                    ...
                    let type = TrailLocationType(rawValue: site.unwrappedTypeString)!
    ...
    func fetchLocations(inputLocations: [TrailLocationType]) -> [TrailLocation] {
        var items = [TrailLocation]()
        //        let tables = ["Campsites", "Shelters", "Vistas", "Privies"]
